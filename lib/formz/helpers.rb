...
module Formz
  ...
  module Helpers
    ...
    def create_tag name, contents, attrs, &block
      attrs[:enctype] = @__form_encoding if name == :form && @__form_encoding
      super
    end
    ...
    def group name, &block
      tag :div, { :class => "group-#{name}" }, &block
    end
    ...
    def file name, value = nil, attrs = {}
      attrs, value = value, nil if value.is_a?(Hash)
      attrs[:value] ||= value
      @__form_encoding = 'multipart/form-data' 
      tag :input, {:type => :file, :name => name}.merge(attrs)
    end

    %w( checkbox radio text submit reset hidden button password ).each do |name|
      class_eval <<-END
    ...
    alias_method :select_tag, :select
