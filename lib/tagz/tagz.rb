...
module Tagz
  ...
  class Tag < BasicObject
    ...
    def initialize name, contents = nil, attrs = {}, context = nil, &block
    ...
    def to_s
      @context.create_tag name, contents, attrs, &proc
    end

    def method_missing(name, *args, &blk)
      @contents << @context.send(name, *args, &blk)
    end
  end
  ...
  def tag name, contents = "", attrs = {}, &block
    attrs, contents = contents, "" if contents.is_a? Hash
    Tag.new(name, contents, attrs, self, &block).to_s
  end
