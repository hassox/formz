...
describe Formz do
  describe "faux method" do
    it "should add a hidden _method field when verb is not POST or GET" do
      formz.form(:save, :method => :put).should have_tag('form[@method=post]') do |form|
    ...
    it "should not duplicate fields" do
      formz.form(:save, :method => :put).should have_tag('input[@name=_method]', :times => 1)
    end

    it "should not add a hidden field when GET or POST" do
      formz.form(:search, :method => :post).should_not have_tag('input[@type=hidden]')
      formz.form(:search, :method => :get).should_not have_tag('input[@type=hidden]')
    end
  end
end
