  ...
  describe "helpers" do
    describe "#select" do
      it "should create a select element with options" do
        markup = formz.select :province, :ab => 'Alberta', :bc => 'British Columbia'
      ...
      it "should create a select with optgroups" do
        ...
        options[:other] = 'Other'
        markup = formz.select :province, options, :selected => :ab
        markup.should have_tag('select') do |select|
      ...
      it "should allow a null prompt with a default" do
        markup = formz.select :vehicle, { :nissan => 'Nissan' }, :prompt => true
        markup.should have_tag('option[@value=""]', '- Select -')
      end

      it "should allow an arbitrary prompt" do
        markup = formz.select :vehicle, { :nissan => 'Nissan' }, :prompt => 'Please Select'
        markup.should have_tag('option[@value=""]', 'Please Select')
      end

      it "should allow selections by value" do
        markup = formz.select :province, { :ab => 'Alberta', :bc => 'British Columbia' }, :selected => :bc, :prompt => true
      ...
      it "should allow selections of symbols / strings" do
        markup = formz.select :province, { 'ab' => 'Alberta', :bc => 'British Columbia' }, :selected => [:ab, 'bc'], :prompt => true
        markup.should have_tag('option[@value=bc]') do |option|
          option['selected'].should == 'selected'
        end
      ...
      it "should allow multiple selections" do
        ...
        markup = formz.select :vehicle, options, :selected => [:nissan, :acura]
    ...
    describe "#form" do
      it "should add a form-ID" do
        formz.form(:login).should have_tag('form[@id=form-login]')
      end
    end

    describe "#fieldset" do
      it "should add a legend when string passed as second arg" do
        markup = formz.form :register do
      ...
      it "should allow hash of attributes, without legend" do
        markup = formz.fieldset :details, :id => :foo
    ...
    describe "#legend" do
      it "should create a legend tag" do
        formz.legend('Details', :id => 'user-details').should have_tag('legend[@id=user-details]', 'Details')
      end
    end

    describe "#buttons" do
      it "should wrap in a buttons div" do
        formz.buttons(:id => 'login-buttons').should have_tag('div[@id=login-buttons]') do |div|
    ...
    describe "#radio_group" do
      it "should create a radio button group" do
        markup = formz.radio_group :choice, { :yes => 'Yes', :no => 'No' }, :selected => :no
      ...
      it "should allow multiple selections (though not legal)" do
        markup = formz.radio_group :choice, { :yes => 'Yes', :no => 'No' }, :selected => [:no, :yes]
