...
describe Tagz do
  describe "#tag" do
    it "should return a simple tag when only a tag name is passed" do
      formz.tag(:div).should have_tag(:div)
    end

    it "should self-close appropriate tags automatically" do
      formz.tag(:br).should include('<br/>')
    end

    it "should allow a hash of attributes to be passed" do
      formz.tag(:input, :type => :text).should have_tag('input[@type=text]')
    end

    it "should allow contents to be passed" do
      formz.tag(:textarea, 'hello', :id => 'comments').should have_tag('textarea[@id=comments]', 'hello')
    end

    it "should mirror boolean attributes" do
      formz.tag(:input, :type => :checkbox, :checked => true).should have_tag('input[@checked=checked]')
      formz.tag(:input, :type => :checkbox, :checked => false).should_not have_tag('input[@checked=checked]')
    end

    it "should accept blocks to populate contents" do
      markup = formz.tag :form do |div|
        div.tag :h2, 'Login'
